...
public:
    ...
    void folklore_compare(){
        ...
        Plaintext a; 
        {
            ...
            //for(auto e:bit_reps){std::cout<<e<<" ";}std::cout<<std::endl;
        ...
        evaluator->square_inplace(eq);//(a-b)^2
        evaluator->relinearize_inplace(eq, *rlk_server);
        evaluator->negate_inplace(eq);//-(a-b)^2
        evaluator->add_plain_inplace(eq, one);//1-(a-b)^2
        evaluator->add_plain(neg_b, one, gt);//1-b
        evaluator->multiply_plain_inplace(gt, a);//(1-b)*a
