...
public:
    ...
    Ciphertext great_than(vector<Plaintext>& raw_a, vector<Ciphertext>& raw_b) override {
        auto a = raw_a[0];
        auto b = raw_b[0];
    ...
    vector<vector<uint64_t>> raw_encode_b(const vector<uint64_t>& in) override {
        ...
        for(uint64_t i = 0; i < num_cmps_per_row; ++i){
            uint64_t val_low = in[i];
            uint64_t val_high = in[num_cmps_per_row + i];
            uint64_t base_offset = i * num_slots_per_element;

            for(int j = 0; j < n; ++j){ 
                out[base_offset + j] = (val_low >> j) & 1;
                out[row_count + base_offset + j] = (val_high >> j) & 1;
            }
        }
        return vector<vector<uint64_t>>{out};
    }
