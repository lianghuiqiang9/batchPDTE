...
class PDTE {
    ...
    void setup_cmp(int cmp_type, int l, int m, int n = 0, int extra = 0){
        ...
        one_one_one = cmp->lhe->encode(one);

        vector<uint64_t> zero(cmp->slot_count, 0ULL);
        zero_zero_zero = cmp->lhe->encrypt(zero);
    ...
    LeafFlatten encode_tree(shared_ptr<Node> root){
        ...
        for(int i = 0; i<leaf_vec.size(); i++){
            ...
            leaf_vec_pt[i] = cmp->lhe->encode(leaf);
        }
    ...
    Ciphertext evaluate(shared_ptr<Node> root, vector<vector<Ciphertext>>& data_cipher, LeafFlatten& leaf_flatten, vector<vector<uint64_t>>& data){
        ...
        while (!stk.empty()) {
            ...
            if (!frame.visited) {
                ...
                cmp->lhe->negate(node->right->value, node->left->value);
                cmp->lhe->add_plain_inplace(node->left->value, one_one_one);
                cmp->lhe->add_inplace(node->left->value, node->value);
                cmp->lhe->add_inplace(node->right->value, node->value);
        ...
        // auto zero_temp_pt = cmp->lhe->decrypt(zero_zero_zero);
        // auto zero_temp = cmp->lhe->decode(zero_temp_pt);
    ...
    Plaintext init_d_factorial_inv_pt(){
        auto d_factorial_inv = d_factorial_inv_with_sign(tree_depth, cmp->lhe->plain_modulus);
        vector<uint64_t> d_factorial_inv_vec(cmp->slot_count, d_factorial_inv);
        return cmp->lhe->encode(d_factorial_inv_vec);
    }

    vector<Plaintext> init_tree_depth_vec(){
        vector<Plaintext> temp(tree_depth);
        auto slot_count = cmp->lhe->slot_count;
        for(int i = 0; i < tree_depth; i++){
            vector<uint64_t> vec_temp(slot_count, i + 1);
            temp[i] = cmp->lhe->encode(vec_temp);
        }
        return temp;
    }
    ...
    vector<Ciphertext> map_to_boolean(vector<Ciphertext>& a){
        for(int i = 0; i < a.size(); i++){
            ...
            for(int j = 0; j < tree_depth; ++j){
                temp[j] =  cmp->lhe->sub_plain(a[i], tree_depth_vec_pt[j]);
            }

            a[i] = cmp->lhe->multiply_many(temp);
            cmp->lhe->multiply_plain_inplace(a[i], tree_depth_factorial_inv_pt);
    ...
    Ciphertext private_info_retrieval(vector<Ciphertext> a, vector<Plaintext> b, vector<uint64_t> b_vec){
        for(int i = 0; i < a.size(); ++i){
            if(b_vec[i] != 0){ //seal ciphertext can not mult zero
                ...
                cmp->lhe->multiply_plain_inplace(a[i], b[i]);
            }else{
                a[i] = zero_zero_zero;
            }
        }
        ...
        return cmp->lhe->add_many(a);
    }

    vector<uint64_t> recovery(Ciphertext& a){
        return cmp->recovery(a);
    }
