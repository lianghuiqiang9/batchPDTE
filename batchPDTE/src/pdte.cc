...
// server
LeafFlatten PDTE::encode_tree(shared_ptr<Node> root){
...
vector<vector<Plaintext>> PDTE::init_salts(int row, int cols){
...
vector<vector<Ciphertext>> PDTE::evaluate(shared_ptr<Node> root, vector<vector<Ciphertext>>& data_cipher, LeafFlatten& leaf_flatten){
...
vector<vector<Ciphertext>> PDTE::extended_sum_path(vector<Ciphertext>& sum_path_result, LeafFlatten& leaf_flatten){
...
vector<vector<Ciphertext>> PDTE::shuffle_result(vector<vector<Ciphertext>> out, int leaf_num){
...
vector<vector<Ciphertext>> PDTE::adapted_sum_path(vector<Ciphertext>& sum_path_result, LeafFlatten& leaf_flatten){
...
// [0, ..., d-1] --> [0,1]
vector<Ciphertext> PDTE::map_to_boolean(vector<Ciphertext>& a){
...
Ciphertext PDTE::private_info_retrieval(vector<Ciphertext> a, vector<Plaintext> b, vector<uint64_t> b_vec){
    for(size_t i = 0; i < a.size(); ++i){
...
vector<uint64_t> PDTE::recover_shuffle_result(vector<vector<Ciphertext>>& a){
