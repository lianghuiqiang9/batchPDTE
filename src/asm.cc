...
// client
void ASM::setup_cmp(int cmp_type, int n, int m, int extra){
    
    int log_tree_depth = static_cast<int>(std::ceil(std::log2(tree_depth) + 1));
        
    switch (cmp_type) {
        case 0: cmp = make_unique<TCMP>(n, m, log_tree_depth + extra); break;
        case 1: cmp = make_unique<DCMP>(n, m, log_tree_depth + extra); break;
        case 2: cmp = make_unique<CWCMP>(n, m, log_tree_depth + extra); break;
    }
