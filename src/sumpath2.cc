...
// client
void SumPath2::setup_cmp(int cmp_type, int n, int m, int extra){
    if (n!=m){
        cout<<"please choose the n == m, get the best performance."<<endl;
    }

    switch (cmp_type) {
        case 0: cmp = make_unique<TCMP>(n, m, extra, true); break;
        case 1: cmp = make_unique<DCMP>(n, m, extra, true); break;
    }
