...
CWCMP::CWCMP(int l, int m, int extra, bool is_rotate) {
    /////////////////////////////////
    this->scheme = "cwcmp";
...
vector<vector<uint64_t>> CWCMP::encode_b(const vector<vector<uint64_t>>& raw_b) {
    vector<vector<uint64_t>> out(encoder.l);
    for (uint64_t j = 0; j < encoder.l; ++j) {
...
Ciphertext CWCMP::piecewise_comp_rec(vector<Ciphertext>& b, vector<uint64_t>& a, uint64_t h) {
    ...
    if (c0 == -1) return zero_zero_zero_cipher;
...
Ciphertext CWCMP::great_than(vector<vector<uint64_t>>& raw_a, vector<Ciphertext>& b) {
    auto a = raw_a[0];
    //print_vector(a, encoder.l, "a:");

    return piecewise_comp_rec(b,a,encoder.h);
...
vector<Plaintext> CWCMP::init_h_factorial_inv_pt(){
    vector<Plaintext> out(encoder.h);
    for(uint64_t i = 0; i < encoder.h; i++){
...
vector<Plaintext> CWCMP::init_h_vec(){
    vector<Plaintext> temp(encoder.h);
    for(uint64_t i = 0; i < encoder.h; i++){
