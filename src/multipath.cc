...
// client
void MultiPath::setup_cmp(int cmp_type, int n, int m, int extra){
    if (n!=m){
    ...
    //}
    cmp = make_unique<DCMP>(n, m, extra, true);
...
vector<vector<Ciphertext>> MultiPath::evaluate(shared_ptr<Node> root, vector<vector<Ciphertext>>& data_cipher, TreeFlatten& tree_flatten){
    ...
    auto out = lhe->multiply_many(cmp_out); // a different of sumpath
    // pir
