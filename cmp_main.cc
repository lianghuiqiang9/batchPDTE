...
int main(int argc, char* argv[]) {
    ...
    while ((opt = getopt(argc, argv, "fn:m:h:c:e:")) != -1) {
        switch (opt) {
    ...
    switch (cmp_type) {
        case 0: cmp = make_unique<TCMP>(n, m, extra); break;
        case 1: cmp = make_unique<DCMP>(n, m, extra); break;
        case 2: cmp = make_unique<CWCMP>(n, m, extra); break;
    }
    ...
    cmp->print();
    cout<< " decrypt result                           : "<< is_correct 
        << " \n keys size                                : "<< (float)cmp->keys_size()/1024
        << " KB\n batch nums                               : "<< num_cmps
        << " \n cmp time cost                            : "<< (float)finish/1000     
        << " ms\n cmp comm. cost                           : "<< (float)comm/1024 //
